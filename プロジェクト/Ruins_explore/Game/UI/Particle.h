...
public:
	...
	void SetValue(int val) { SetValue((float)val); }
	// 画面上の位置を設定
	...
	void SetSize(float x, float y) { SetSize(DirectX::SimpleMath::Vector2(x, y)); }
	// 数字の色を設定
	void SetNumberColor(DirectX::SimpleMath::Vector4 colorA, DirectX::SimpleMath::Vector4 colorB);
	// 背景色を設定
	void SetBackColor(DirectX::SimpleMath::Vector4 color) { m_colorC = color; }
