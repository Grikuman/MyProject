...
void Scene::Render()
{
	...
	{
		// 配置決め
		m_positions[i] = Vector3{ 0,0,1.5f };

		// 回転する
		world = Matrix::CreateTranslation(m_positions[i]);
		world *= Matrix::CreateRotationY(XMConvertToRadians(90.0f * i + m_rotation) * -8.0f);
		world *= Matrix::CreateTranslation(m_positions[i - 4]);
		world *= Matrix::CreateRotationY(XMConvertToRadians(90.0f * i + m_rotation));
		/*
			・自身を原点から平行移動する
			・Ｙ軸を中心に回転：親より早く回転しないと雰囲気が出ない…
			・親子関係を持たせる
			・親と同じ分だけ回転
		*/
