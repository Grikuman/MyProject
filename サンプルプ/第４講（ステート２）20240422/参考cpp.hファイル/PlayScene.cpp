...
// 初期位置
const DirectX::SimpleMath::Vector3 PlayScene::HOME_POSITION = DirectX::SimpleMath::Vector3(0.0f, 47.0f, 0.0f);
// 地面
const float PlayScene::GROUND = 400.0f;

// コンストラクタ
PlayScene::PlayScene(Game* game)
	:
	m_game(game),									// Gameクラス
	m_graphics(nullptr),							// Graphicsクラス
...
{
	// Marioオブジェクトを生成する
	m_mario = std::make_unique<Mario>(this);
	// Marioオブジェクトを初期化する
	m_mario->Initialize();
...
void PlayScene::Update(const DX::StepTimer& timer)
{
	...
	// キーボードの状態を更新する
	m_game->GetKeyboardStateTracker().Update(m_keyboardState);
	// マウスの状態を取得する
	...
	// マリオのキーボードステートを渡す
	m_mario->Update(timer, m_game->GetKeyboardStateTracker());
...
// プロジェクションを生成する
void PlayScene::CreateProjection()
{
	// ウィンドウサイズを取得する
...
void PlayScene::SetWindow(const int& width, const int& height)
{
